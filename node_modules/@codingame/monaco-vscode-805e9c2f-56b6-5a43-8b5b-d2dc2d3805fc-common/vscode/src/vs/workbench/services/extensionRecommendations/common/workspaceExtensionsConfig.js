...
import { isWorkspace } from '@codingame/monaco-vscode-api/vscode/vs/platform/workspace/common/workspace';
...
let WorkspaceExtensionsConfigService = class WorkspaceExtensionsConfigService extends Disposable {
    ...
    async toggleRecommendation(extensionId) {
        ...
        const workspaceOrFolders = isRecommended
            ? await this.pickWorkspaceOrFolders(recommendedWorksapceFolders, isWorkspaceRecommended ? workspace : undefined, ( localize(11819, "Remove extension recommendation from")))
            : await this.pickWorkspaceOrFolders(workspace.folders, workspace.configuration ? workspace : undefined, ( localize(11820, "Add extension recommendation to")));
    ...
    async toggleUnwantedRecommendation(extensionId) {
        ...
        const workspaceOrFolders = isUnwanted
            ? await this.pickWorkspaceOrFolders(unWantedWorksapceFolders, isWorkspaceUnwanted ? workspace : undefined, ( localize(11819, "Remove extension recommendation from")))
            : await this.pickWorkspaceOrFolders(workspace.folders, workspace.configuration ? workspace : undefined, ( localize(11820, "Add extension recommendation to")));
        for (const workspaceOrWorkspaceFolder of workspaceOrFolders) {
            if (isWorkspace(workspaceOrWorkspaceFolder)) {
    ...
    async pickWorkspaceOrFolders(workspaceFolders, workspace, placeHolder) {
        ...
        const folderPicks = ( workspaceFolders.map(workspaceFolder => {
            return {
                ...
                description: ( localize(11821, "Workspace Folder")),
        ...
        if (workspace) {
            ...
            folderPicks.push({
                label: ( localize(11822, "Workspace")),
