...
import { commentViewThreadStateColorVar, getCommentThreadStateIconColor } from '@codingame/monaco-vscode-a3f28a41-ba19-5a7e-8f5a-d6c1403b507d-common/vscode/vs/workbench/contrib/comments/browser/commentColors';
...
const COMMENTS_VIEW_TITLE = ( localize2(5125, "Comments"));
...
let CommentNodeRenderer = class CommentNodeRenderer {
    ...
    getCountString(commentCount) {
        if (commentCount > 2) {
            return localize(5126, "{0} replies", commentCount - 1);
        }
        else if (commentCount === 2) {
            return localize(5127, "1 reply");
        }
        else {
            return localize(5128, "1 comment");
        }
    }
    getRenderedComment(commentBody, disposables) {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            textDescription.textContent = image.alt ? ( localize(5129, "Image: {0}", image.alt)) : ( localize(5130, "Image"));
    ...
    renderElement(node, index, templateData, height) {
        ...
        if (node.element.threadRelevance === CommentThreadApplicability.Outdated) {
            ...
            templateData.threadMetadata.relevance.innerText = ( localize(5131, "Outdated"));
        ...
        if (node.element.range) {
            if (node.element.range.startLineNumber === node.element.range.endLineNumber) {
                templateData.threadMetadata.range.textContent = ( localize(5132, "[Ln {0}]", node.element.range.startLineNumber));
            }
            else {
        ...
        templateData.repliesMetadata.lastReplyDetail.textContent = ( localize(5134, "Last reply from {0}", lastComment.userName));
