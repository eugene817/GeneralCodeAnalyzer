...
import '@codingame/monaco-vscode-a3f28a41-ba19-5a7e-8f5a-d6c1403b507d-common/vscode/vs/workbench/contrib/comments/browser/media/review.css';
...
import { COMMENTS_SECTION } from '@codingame/monaco-vscode-a3f28a41-ba19-5a7e-8f5a-d6c1403b507d-common/vscode/vs/workbench/contrib/comments/common/commentsConfiguration';
import { COMMENTEDITOR_DECORATION_KEY } from '@codingame/monaco-vscode-a3f28a41-ba19-5a7e-8f5a-d6c1403b507d-common/vscode/vs/workbench/contrib/comments/browser/commentReply';
    ...
    _findNearestCommentingRange(reverse) {
        ...
        if (this.accessibilityService.isScreenReaderOptimized()) {
            ...
            if (commentRangeStart && commentRangeEnd) {
                ...
                oneLine ? status(( localize(5104, "Line {0}", commentRangeStart))) : status(( localize(5105, "Lines {0} to {1}", commentRangeStart, commentRangeEnd)));
            }
        }
    }
    nextCommentingRange() {
            ...
            if (!this._hasRespondedToEditorChange) {
                if (( this._commentInfos.some(
                    ...
                    if (verbose) {
                        ...
                        status(( localize(5108, "Editor has commenting ranges.")));
                    }
                }
            }
        });
    ...
    addCommentAtLine(range, e) {
        ...
        if (!newCommentInfos.length || !this.editor?.hasModel()) {
            ...
            if (!newCommentInfos.length) {
                if (range) {
                    this.notificationService.error(( localize(5109, "The cursor must be within a commenting range to add a comment.")));
                }
                else {
                    this.notificationService.error(( localize(5110, "File comments are not allowed on this file.")));
                }
            }
            return Promise.resolve();
        }
        if (newCommentInfos.length > 1) {
            if (e && range) {
                ...
                return this.quickInputService.pick(picks, { placeHolder: ( localize(5111, "Select Comment Provider")), matchOnDescription: true }).then(pick => {
                    if (!pick) {
                        return;
                    }
