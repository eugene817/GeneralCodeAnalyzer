...
import { TriggerAction } from '@codingame/monaco-vscode-12c5f9eb-72d3-57ca-babd-5bef7aa9de3b-common/vscode/vs/platform/quickinput/browser/pickerQuickAccess';
...
let CommandsQuickAccessProvider = class CommandsQuickAccessProvider extends AbstractEditorCommandsQuickAccessProvider {
    ...
    constructor(editorService, menuService, extensionService, instantiationService, keybindingService, commandService, telemetryService, dialogService, configurationService, editorGroupService, preferencesService, productService, aiRelatedInformationService, chatAgentService) {
        super({
            ...
            noResultsPick: () => ({
                label: ( localize(8473, "No matching commands")),
                commandId: ''
            }),
    ...
    async getCommandPicks(token) {
        ...
        return ([
            ...
            buttons: [{
                    ...
                    tooltip: ( localize(8474, "Configure Keybinding")),
                }],
            trigger: () => {
    ...
    async getAdditionalCommandPicks(allPicks, picksSoFar, filter, token) {
        ...
        if (defaultAgent) {
            additionalPicks.push({
                label: ( localize(8475, "Ask {0}: {1}", defaultAgent.fullName, filter)),
    ...
    getGlobalCommandPicks() {
        ...
        for (const action of globalCommandsMenuActions) {
            ...
            if (category) {
                label = ( localize(8476, "{0}: {1}", category, label));
            }
...
class ShowAllCommandsAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(8477, 'Show All Commands')),
            keybinding: {
...
class ClearCommandHistoryAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(8478, 'Clear Command History')),
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (commandHistoryLength > 0) {
            const { confirmed } = await dialogService.confirm({
                type: 'warning',
                message: ( localize(8479, "Do you want to clear the history of recently used commands?")),
                detail: ( localize(8480, "This action is irreversible!")),
                primaryButton: ( localize(8481, "&&Clear"))
            });
            if (!confirmed) {
                return;
            }
