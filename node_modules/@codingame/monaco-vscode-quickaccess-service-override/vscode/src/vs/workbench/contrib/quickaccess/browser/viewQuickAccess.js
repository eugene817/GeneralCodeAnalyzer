...
let ViewQuickAccessProvider = class ViewQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    constructor(viewDescriptorService, viewsService, outputService, terminalService, terminalGroupService, debugService, paneCompositeService, contextKeyService) {
        super(ViewQuickAccessProvider_1.PREFIX, {
            noResultsPick: {
                label: ( localize(8494, "No matching views")),
                containerLabel: ''
            }
        });
    ...
    doGetViewPickItems() {
        ...
        addPaneComposites(ViewContainerLocation.Sidebar, ( localize(8495, "Side Bar")));
        addPaneComposites(ViewContainerLocation.Panel, ( localize(8496, "Panel")));
        addPaneComposites(ViewContainerLocation.AuxiliaryBar, ( localize(8497, "Secondary Side Bar")));
        ...
        this.terminalGroupService.groups.forEach((group, groupIndex) => {
            group.terminalInstances.forEach((terminal, terminalIndex) => {
                const label = ( localize(8498, "{0}: {1}", `${groupIndex + 1}.${terminalIndex + 1}`, terminal.title));
                viewEntries.push({
                    label,
                    containerLabel: ( localize(8499, "Terminal")),
                    accept: async () => {
        ...
        this.debugService.getModel().getSessions(true).filter(s => s.hasSeparateRepl()).forEach((session, _) => {
            ...
            viewEntries.push({
                label,
                containerLabel: ( localize(8500, "Debug Console")),
                accept: async () => {
        ...
        for (const channel of channels) {
            viewEntries.push({
                label: channel.label,
                containerLabel: ( localize(8501, "Output")),
...
class OpenViewPickerAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(8502, 'Open View')),
...
class QuickAccessViewPickerAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(8503, 'Quick Open View')),
            category: Categories.View,
            f1: false,
            keybinding: {
