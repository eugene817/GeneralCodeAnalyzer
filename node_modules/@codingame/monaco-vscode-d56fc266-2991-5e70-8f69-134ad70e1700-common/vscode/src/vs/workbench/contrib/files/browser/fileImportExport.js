...
import { getFileNamesMessage } from '@codingame/monaco-vscode-2e69e120-617a-5258-95e0-3b8902f4e014-common/vscode/vs/platform/dialogs/common/dialogs';
...
import { VIEW_ID, UndoConfirmLevel } from '@codingame/monaco-vscode-a4c2011e-8775-52bd-abf0-4a3c07a9696b-common/vscode/vs/workbench/contrib/files/common/files';
            ...
            return uploadLimiter.queue(async () => {
                ...
                if (target && entry.name && target.getChild(entry.name)) {
                    ...
                    await this.explorerService.applyBulkEdit([( new ResourceFileEdit(
                        ...
                        undoLabel: ( localize(6781, "Overwrite {0}", entry.name)),
                        progressLabel: ( localize(6782, "Overwriting {0}", entry.name)),
                    });
                    if (token.isCancellationRequested) {
                        return;
                    }
    ...
    async doImport(target, source, targetWindow, token) {
        ...
        if (folders.length > 0 && target.isRoot) {
            ...
            const buttons = [
                {
                    label: folders.length > 1 ?
                        ( localize(6786, "&&Copy Folders")) :
                        ( localize(6787, "&&Copy Folder")),
            ...
            if (( folders.some(folder => workspaceFolderSchemas.indexOf(folder.uri.scheme) >= 0))) {
                buttons.unshift({
                    label: folders.length > 1 ?
                        ( localize(6788, "&&Add Folders to Workspace")) :
                        ( localize(6789, "&&Add Folder to Workspace")),
                ...
                message = folders.length > 1 ?
                    ( localize(6792, "Are you sure to want to copy folders?")) :
                    ( localize(6793, "Are you sure to want to copy '{0}'?", basename(folders[0].uri)));
            }
    ...
    async importResources(target, resources, token) {
        if (resources && resources.length > 0) {
            ...
            if (inaccessibleFileCount > 0) {
                this.notificationService.error(inaccessibleFileCount > 1 ? ( localize(6794, "Some or all of the dropped files could not be accessed for import.")) : ( localize(6795, "The dropped file could not be accessed for import.")));
            }
            ...
            await this.explorerService.applyBulkEdit(resourceFileEdits, {
                undoLabel: resourcesFiltered.length === 1 ?
                    ( localize(6796, "Import {0}", basename(resourcesFiltered[0]))) :
                    ( localize(6797, "Import {0} resources", resourcesFiltered.length)),
                progressLabel: resourcesFiltered.length === 1 ?
                    ( localize(6798, "Copying {0}", basename(resourcesFiltered[0]))) :
                    ( localize(6799, "Copying {0} resources", resourcesFiltered.length)),
...
let FileDownload = class FileDownload {
    ...
    download(source) {
        ...
        const downloadPromise = this.progressService.withProgress({
            ...
            delay: 800,
            cancellable: isWeb,
            title: ( localize(6800, "Downloading"))
    ...
    async doDownloadNative(explorerItem, progress, cts) {
        ...
        const destination = await this.fileDialogService.showSaveDialog({
            ...
            saveLabel: ( localize(6803, "Download")),
            title: ( localize(6804, "Choose Where to Download")),
            defaultUri
        });
        if (destination) {
            ...
            await this.explorerService.applyBulkEdit([( new ResourceFileEdit(explorerItem.resource, destination, { overwrite: true, copy: true }))], {
                undoLabel: ( localize(6805, "Download {0}", explorerItem.name)),
                progressLabel: ( localize(6806, "Downloading {0}", explorerItem.name)),
                progressLocation: ProgressLocation.Window
            });
        }
        else {
...
function getFileOverwriteConfirm(name) {
    return {
        ...
        detail: ( localize(6808, "This action is irreversible!")),
        primaryButton: ( localize(6809, "&&Replace")),
        type: 'warning'
    };
}
function getMultipleFilesOverwriteConfirm(files) {
    if (files.length > 1) {
        return {
            ...
            detail: getFileNamesMessage(files) + '\n' + ( localize(6808, "This action is irreversible!")),
            primaryButton: ( localize(6809, "&&Replace")),
            type: 'warning'
        };
    }
