...
let DefaultFormatter = class DefaultFormatter extends Disposable {
    ...
    async _updateConfigValues() {
        ...
        DefaultFormatter_1.extensionItemLabels.push(( localize(6977, 'None')));
        DefaultFormatter_1.extensionDescriptions.push(( localize(6978, "None")));
    ...
    async _selectFormatter(formatter, document, mode, kind) {
        ...
        if (mode !== FormattingMode.Silent) {
            const { confirmed } = await this._dialogService.confirm({
                message: ( localize(6983, "Configure Default Formatter")),
                detail: formatterOrMessage,
                primaryButton: ( localize(6984, "&&Configure..."))
            });
            if (confirmed) {
            ...
            this._notificationService.prompt(Severity$1.Info, formatterOrMessage, [{ label: ( localize(6985, "Configure...")), run: () => this._pickAndPersistDefaultFormatter(formatter, document) }], { priority: NotificationPriority.SILENT });
        }
        return undefined;
    }
    ...
    _updateStatus() {
        ...
        this._analyzeFormatter(FormattingKind.File, formatter, document).then(result => {
            ...
            const command = { id: `formatter/configure/dfl/${generateUuid()}`, title: ( localize(6987, "Configure...")) };
...
async function showFormatterPick(accessor, model, formatters) {
    ...
    const picks = ( formatters.map((provider, index) => {
        ...
        const pick = {
            ...
            description: isDefault ? ( localize(6991, "(default)")) : undefined,
        };
        if (isDefault) {
    ...
    const configurePick = {
        label: ( localize(6992, "Configure Default Formatter..."))
    };
    const pick = await quickPickService.pick([...picks, { type: 'separator' }, configurePick], {
        placeHolder: ( localize(6993, "Select a formatter")),
...
registerEditorAction(class FormatDocumentMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize(6994, "Format Document With...")),
...
registerEditorAction(class FormatSelectionMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize(6995, "Format Selection With...")),
            alias: 'Format Code...',
