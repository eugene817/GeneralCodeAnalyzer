...
const configurationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(12158, 'Contributes extension defined themable colors')),
        type: 'array',
        items: {
            type: 'object',
            properties: {
                id: {
                    type: 'string',
                    description: ( localize(12159, 'The identifier of the themable color')),
                ...
                description: {
                    type: 'string',
                    description: ( localize(12161, 'The description of the themable color')),
                },
                defaults: {
                    type: 'object',
                    properties: {
        ...
        configurationExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(12166, "'configuration.colors' must be a array")));
                    return;
                }
                ...
                for (const colorContribution of extensionValue) {
                    if (typeof colorContribution.id !== 'string' || colorContribution.id.length === 0) {
                        collector.error(( localize(12168, "'configuration.colors.id' must be defined and can not be empty")));
                        return;
                    }
...
class ColorDataRenderer extends Disposable {
    ...
    render(manifest) {
        ...
        const headers = [
            ( localize(12174, "ID")),
            ( localize(12175, "Description")),
            ( localize(12176, "Dark Default")),
            ( localize(12177, "Light Default")),
            ( localize(12178, "High Contrast Default")),
        ];
