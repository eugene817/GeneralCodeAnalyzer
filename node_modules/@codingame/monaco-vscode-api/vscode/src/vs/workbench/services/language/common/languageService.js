...
const languagesExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(11987, 'Contributes language declarations.')),
        type: 'array',
        items: {
            type: 'object',
            ...
            properties: {
                id: {
                    description: ( localize(11988, 'ID of the language.')),
                    type: 'string'
                },
                aliases: {
                    description: ( localize(11989, 'Name aliases for the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                extensions: {
                    description: ( localize(11990, 'File extensions associated to the language.')),
                    default: ['.foo'],
                    type: 'array',
                    items: {
                        type: 'string'
                ...
                filenames: {
                    description: ( localize(11991, 'File names associated to the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                filenamePatterns: {
                    description: ( localize(11992, 'File name glob patterns associated to the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                mimetypes: {
                    description: ( localize(11993, 'Mime types associated to the language.')),
                    type: 'array',
                    items: {
                        type: 'string'
                    }
                ...
                icon: {
                    ...
                    properties: {
                        light: {
                            description: ( localize(11997, 'Icon path when a light theme is used')),
                            type: 'string'
                        },
                        dark: {
                            description: ( localize(11998, 'Icon path when a dark theme is used')),
                            type: 'string'
                        }
                    }
                }
...
class LanguageTableRenderer extends Disposable {
    ...
    render(manifest) {
        ...
        const headers = [
            ( localize(11999, "ID")),
            ( localize(12000, "Name")),
            ( localize(12001, "File Extensions")),
            ( localize(12002, "Grammar")),
            ( localize(12003, "Snippets"))
        ];
...
( Registry.as(Extensions.ExtensionFeaturesRegistry)).registerExtensionFeature({
    id: 'languages',
    label: ( localize(12004, "Programming Languages")),
    access: {
        canToggle: false
    },
...
function isValidLanguageExtensionPoint(value, collector) {
    if (!value) {
        collector?.error(( localize(12006, "Empty value for `contributes.{0}`", languagesExtPoint.name)));
        return false;
    }
    if (typeof value.id !== 'string') {
        collector?.error(( localize(12007, "property `{0}` is mandatory and must be of type `string`", 'id')));
        return false;
    }
