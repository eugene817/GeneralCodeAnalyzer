    ...
    async $startChatRequest(handle, requestId, from, messages, options, token) {
        ...
        try {
            if (data.provider.provideLanguageModelResponse2) {
                value = data.provider.provideLanguageModelResponse2(( messages.map(LanguageModelChatMessage.to)), options, ExtensionIdentifier.toKey(from), progress, token);
            }
            else {
                value = data.provider.provideLanguageModelResponse(( messages.map(LanguageModelChatMessage.to)), options, ExtensionIdentifier.toKey(from), progress, token);
            }
        }
        catch (err) {
            throw err;
