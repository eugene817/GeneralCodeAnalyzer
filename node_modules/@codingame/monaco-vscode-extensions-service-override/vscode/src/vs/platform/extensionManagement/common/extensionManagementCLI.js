...
const notFound = (id) => ( localize(1736, "Extension '{0}' not found.", id));
...
let ExtensionManagementCLI = class ExtensionManagementCLI {
    ...
    async listExtensions(showVersions, category, profileLocation) {
        ...
        if (this.location) {
            this.logger.info(( localize(1738, "Extensions installed on {0}:", this.location)));
        }
    ...
    async installExtensions(extensions, builtinExtensions, installOptions, force) {
        ...
        try {
            if (extensions.length) {
                this.logger.info(this.location ? ( localize(1739, "Installing extensions on {0}...", this.location)) : ( localize(1740, "Installing extensions...")));
            }
            ...
            this.logger.error(( localize(1741, "Error while installing extensions: {0}", getErrorMessage(error))));
            throw error;
        }
        if (failed.length) {
            throw ( new Error(( localize(1742, "Failed Installing Extensions: {0}", failed.join(', ')))));
        }
    }
    async updateExtensions(profileLocation) {
        ...
        if (!extensionsToUpdate.length) {
            this.logger.info(( localize(1744, "No extension to update")));
            return;
        }
    ...
    async installGalleryExtensions(installExtensionInfos, installed, force) {
        installExtensionInfos = installExtensionInfos.filter(({ id, version }) => {
            ...
            if (installedExtension) {
                ...
                if (version && installedExtension.manifest.version === version) {
                    this.logger.info(( localize(1749, "Extension '{0}' is already installed.", `${id}@${version}`)));
                    return false;
                }
            }
            return true;
        ...
        await Promise.all(( installExtensionInfos.map(async ({ id, version, installOptions }) => {
            ...
            if (installOptions.isBuiltin) {
                this.logger.info(version ? ( localize(1751, "Installing builtin extension '{0}' v{1}...", id, version)) : ( localize(1752, "Installing builtin extension '{0}'...", id)));
            }
            else {
                this.logger.info(version ? ( localize(1753, "Installing extension '{0}' v{1}...", id, version)) : ( localize(1754, "Installing extension '{0}'...", id)));
            }
    ...
    async installVSIX(vsix, installOptions, force, installedExtensions) {
        ...
        if (valid) {
            try {
                ...
                this.logger.info(( localize(1757, "Extension '{0}' was successfully installed.", basename(vsix))));
            }
            catch (error) {
                if (isCancellationError(error)) {
                    this.logger.info(( localize(1758, "Cancelled installing extension '{0}'.", basename(vsix))));
                }
                else {
                    throw error;
                }
    ...
    async uninstallExtensions(extensions, force, profileLocation) {
        ...
        for (const extension of extensions) {
            ...
            this.logger.info(( localize(1762, "Uninstalling {0}...", id)));
            ...
            if (this.location) {
                ...
                this.logger.info(( localize(1764, "Extension '{0}' was successfully uninstalled!", id)));
            }
        }
    }
    ...
    notInstalled(id) {
        return this.location ? ( localize(1765, "Extension '{0}' is not installed on {1}.", id, this.location)) : ( localize(1766, "Extension '{0}' is not installed.", id));
    }
};
