...
let ExtensionBisectUi = class ExtensionBisectUi {
    ...
    _showBisectPrompt() {
        const goodPrompt = {
            label: ( localize(11732, "I can't reproduce")),
            run: () => this._commandService.executeCommand('extension.bisect.next', false)
        };
        const badPrompt = {
            label: ( localize(11733, "I can reproduce")),
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(11736, 'Start Extension Bisect')),
    ...
    async run(accessor) {
        ...
        const res = await dialogService.confirm({
            message: ( localize(11737, "Extension Bisect")),
            detail: ( localize(
                11738,
            ...
            primaryButton: ( localize(11739, "&&Start Extension Bisect"))
        });
        if (res.confirmed) {
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(11740, 'Continue Extension Bisect')),
            category: Categories.Help,
            f1: true,
    ...
    async run(accessor, seeingBad) {
        ...
        if (done.bad) {
            await dialogService.info(( localize(11741, "Extension Bisect")), ( localize(
                11742,
            ...
            const res = await dialogService.confirm({
                type: Severity.Info,
                message: ( localize(11741, "Extension Bisect")),
                primaryButton: ( localize(11743, "&&Report Issue & Continue")),
                cancelButton: ( localize(11744, "Continue")),
                detail: ( localize(
                    11745,
                    "Extension Bisect is done and has identified {0} as the extension causing the problem.",
                    done.id
                )),
                checkbox: { label: ( localize(11746, "Keep this extension disabled")), checked: true }
            });
    ...
    async _checkForBad(dialogService, bisectService) {
        const { result } = await dialogService.prompt({
            type: Severity.Info,
            message: ( localize(11747, "Extension Bisect")),
            detail: ( localize(
                11748,
                "Extension Bisect is active and has disabled {0} extensions. Check if you can still reproduce the problem and proceed by selecting from these options.",
            ...
            buttons: [
                {
                    label: ( localize(11749, "I ca&&n't reproduce")),
                    run: () => false
                },
                {
                    label: ( localize(11750, "I can &&reproduce")),
                    run: () => true
                },
                {
                    label: ( localize(11751, "&&Stop Bisect")),
                    run: () => undefined
                }
            ],
            cancelButton: {
                label: ( localize(11752, "&&Cancel Bisect")),
                run: () => null
            }
        });
        return result;
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(11753, 'Stop Extension Bisect')),
            category: Categories.Help,
            f1: true,
            precondition: ExtensionBisectUi.ctxIsBisectActive
        });
