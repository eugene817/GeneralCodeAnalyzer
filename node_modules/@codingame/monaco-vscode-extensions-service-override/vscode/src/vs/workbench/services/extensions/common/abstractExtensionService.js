        ...
        this._register(this._lifecycleService.onWillShutdown(event => {
            if (this._remoteAgentService.getConnection()) {
                event.join(async () => {
                    ...
                    label: ( localize(11833, "Disconnect Remote Agent")),
                ...
                event.join(this._doStopExtensionHosts(), {
                    ...
                    label: ( localize(11834, "Stopping Extension Hosts")),
                });
            }
        }));
    }
            ...
            veto(value, reason) {
                ...
                if (typeof value === 'boolean') {
                    ...
                    value.then(value => {
                        ...
                        vetoReasons.add(( localize(11837, "{0} (Error: {1})", reason, toErrorMessage(error))));
                    });
                }
            }
        });
        ...
        if (!veto) {
            ...
            if (!auto) {
                ...
                const { confirmed } = await this._dialogService.confirm({
                    ...
                    message: ( localize(11838, "Please confirm restart of extensions.")),
                    detail: vetoReasonsArray.length === 1 ?
                        vetoReasonsArray[0] :
                        vetoReasonsArray.join('\n -'),
                    primaryButton: ( localize(11839, "Restart Anyway"))
                });
                if (confirmed) {
                    return true;
                }
    ...
    async _onRemoteExtensionHostCrashed(extensionHost, reconnectionToken) {
        try {
            ...
            if (this._remoteCrashTracker.shouldAutomaticallyRestart()) {
                ...
                this._notificationService.status(( localize(11840, "The remote extension host terminated unexpectedly. Restarting...")), { hideAfter: 5000 });
                ...
                this._notificationService.prompt(Severity.Error, ( localize(
                    ...
                    "Remote Extension host terminated unexpectedly 3 times within the last 5 minutes."
                        ...
                        label: ( localize(11842, "Restart Remote Extension Host")),
                        run: () => {
                            this._startExtensionHostsIfNecessary(false, Array.from(( this._allRequestedActivateEvents.keys())));
                        }
                    }]);
...
( Registry.as(Extensions.ExtensionFeaturesRegistry)).registerExtensionFeature({
    id: 'activationEvents',
    label: ( localize(11843, "Activation Events")),
    access: {
        canToggle: false
    },
