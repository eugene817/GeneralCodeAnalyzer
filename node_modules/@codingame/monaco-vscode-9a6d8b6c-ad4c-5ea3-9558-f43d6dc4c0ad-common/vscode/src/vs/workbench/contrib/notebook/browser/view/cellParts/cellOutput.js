...
import { $, append, show, hide } from '@codingame/monaco-vscode-api/vscode/vs/base/browser/dom';
...
import { CellUri, isTextStreamMime, RENDERER_NOT_AVAILABLE, NotebookCellExecutionState } from '@codingame/monaco-vscode-api/vscode/vs/workbench/contrib/notebook/common/notebookCommon';
...
import { NOTEBOOK_CELL_HAS_HIDDEN_OUTPUTS, NOTEBOOK_CELL_IS_FIRST_OUTPUT, NOTEBOOK_CELL_OUTPUT_MIMETYPE } from '@codingame/monaco-vscode-52bb4d5b-ba1a-57fd-9bee-b28824214eac-common/vscode/vs/workbench/contrib/notebook/common/notebookContextKeys';
...
let CellOutputElement = class CellOutputElement extends Disposable {
    ...
    _renderMissingRenderer(viewModel, preferredMimeType) {
        if (!viewModel.model.outputs.length) {
            return this._renderMessage(viewModel, ( localize(8024, "Cell has no output")));
        }
    ...
    async _attachToolbar(outputItemDiv, notebookTextModel, kernel, index, mimeTypes) {
        ...
        const pickAction = this.toolbarDisposables.add(( new Action('notebook.output.pickMimetype', ( localize(8026, "Change Presentation")), ThemeIcon.asClassName(mimetypeIcon), undefined, async (_context) => this._pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, this.output))));
    ...
    async _pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, viewModel) {
        ...
        mimeTypes.forEach((mimeType, index) => {
            if (mimeType.isTrusted) {
                ...
                arr.push({
                    ...
                    description: index === currIndex ? ( localize(8027, "Currently Active")) : undefined
                });
            }
        });
        if (( unsupportedItems.some(m => JUPYTER_RENDERER_MIMETYPES.includes(m.id)))) {
            unsupportedItems.push({
                label: ( localize(8028, "Install additional renderers from the marketplace")),
                id: 'installRenderers',
        ...
        picker.placeholder = items.length !== mimeTypes.length
            ? ( localize(8029, "Select mimetype to render for current output"))
            : ( localize(8030, "Select mimetype to render for current output"));
    ...
    _generateRendererInfo(renderId) {
        ...
        return localize(8031, "renderer not available");
    }
