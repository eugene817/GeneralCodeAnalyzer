...
import { $, addStandardDisposableListener, addDisposableListener, EventType, append } from '@codingame/monaco-vscode-api/vscode/vs/base/browser/dom';
...
class OutputElement extends Disposable {
    ...
    _renderMissingRenderer(viewModel, preferredMimeType) {
        if (!viewModel.model.outputs.length) {
            return this._renderMessage(viewModel, ( localize(7804, "Cell has no output")));
        }
    ...
    async pickActiveMimeTypeRenderer(notebookTextModel, viewModel) {
        ...
        const items = ( mimeTypes.filter(mimeType => mimeType.isTrusted).map((mimeType, index) => ({
            ...
            description: index === currIndex ? ( localize(7806, "Currently Active")) : undefined
        })));
        ...
        picker.placeholder = items.length !== mimeTypes.length
            ...
            : ( localize(7808, "Select mimetype to render for current output"));
