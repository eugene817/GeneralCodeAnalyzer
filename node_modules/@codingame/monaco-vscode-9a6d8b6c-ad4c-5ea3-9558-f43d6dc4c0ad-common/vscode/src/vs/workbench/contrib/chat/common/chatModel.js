...
import { ChatAgentLocation, reviveSerializedAgent } from '@codingame/monaco-vscode-api/vscode/vs/workbench/contrib/chat/common/chatAgents';
import { IChatAgentService } from '@codingame/monaco-vscode-api/vscode/vs/workbench/contrib/chat/common/chatAgents.service';
import { reviveParsedChatRequest, ChatRequestTextPart } from './chatParserTypes.js';
import { isIUsedContext } from '@codingame/monaco-vscode-api/vscode/vs/workbench/contrib/chat/common/chatService';
...
(function (IDiagnosticVariableEntryFilterData) {
    ...
    function label(data) {
        ...
        let labelStr = ( localize(4851, "All Problems"));
        if (data.filterUri) {
            labelStr = ( localize(4852, "Problems in {0}", basename(data.filterUri)));
        }
        return labelStr;
    }
...
class ChatRequestModel {
    ...
    constructor(_session, message, _variableData, timestamp, _attempt = 0, _confirmation, _locationData, _attachedContext, isCompleteAddedRequest = false, restoredId) {
        ...
        for (const part of parts) {
            ...
            switch (part.kind) {
                ...
                case 'notebookEditGroup':
                    segment = { text: ( localize(4853, "Made changes.")), isBlock: true };
                    break;
                case 'confirmation':
...
function getCodeCitationsMessage(citations) {
    ...
    const label = licenseTypes.size === 1 ?
        ( localize(4854, "Similar code found with 1 license type", licenseTypes.size)) :
        ( localize(4855, "Similar code found with {0} license types", licenseTypes.size));
    return label;
}

export { ChatModel, ChatModelInitState, ChatPauseState, ChatRequestModel, ChatRequestRemovalReason, ChatResponseModel, IDiagnosticVariableEntryFilterData, Response, appendMarkdownString, canMergeMarkdownStrings, getCodeCitationsMessage, isCellTextEditOperation, isExportableSessionData, isImageVariableEntry, isLinkVariableEntry, isPasteVariableEntry, isSerializableSessionData, normalizeSerializableChatData, toChatHistoryContent, updateRanges };
