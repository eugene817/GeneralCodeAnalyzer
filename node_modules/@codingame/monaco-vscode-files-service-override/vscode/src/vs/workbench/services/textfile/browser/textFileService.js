...
let AbstractTextFileService = class AbstractTextFileService extends Disposable {
    ...
    static { this.TEXTFILE_SAVE_CREATE_SOURCE = SaveSourceRegistry.registerSource('textFileCreate.source', ( localize(12124, "File Created"))); }
    static { this.TEXTFILE_SAVE_REPLACE_SOURCE = SaveSourceRegistry.registerSource('textFileOverwrite.source', ( localize(12125, "File Replaced"))); }
    ...
    provideDecorations() {
        const provider = this._register(new (class extends Disposable {
            constructor(files) {
                super();
                this.files = files;
                this.label = ( localize(12126, "Text File Model Decorations"));
            ...
            provideDecorations(uri) {
                ...
                if (isReadonly && isOrphaned) {
                    return {
                        ...
                        strikethrough: true,
                        tooltip: ( localize(12127, "Deleted, Read-only")),
                    };
                }
                else if (isReadonly) {
                    return {
                        letter: Codicon.lockSmall,
                        tooltip: ( localize(12128, "Read-only")),
                    };
                }
                else if (isOrphaned) {
                    return {
                        color: listErrorForeground,
                        strikethrough: true,
                        tooltip: ( localize(12129, "Deleted")),
                    };
                }
                return undefined;
            }
    ...
    async doRead(resource, options) {
        ...
        try {
            ...
            if (error.decodeStreamErrorKind === DecodeStreamErrorKind.STREAM_IS_BINARY) {
                throw ( new TextFileOperationError(( localize(12130, "File seems to be binary and cannot be opened as text")), TextFileOperationResult.FILE_IS_BINARY, options));
            }
            else {
                throw error;
            }
    ...
    async confirmOverwrite(resource) {
        const { confirmed } = await this.dialogService.confirm({
            ...
            primaryButton: ( localize(12133, "&&Replace")),
        });
        return confirmed;
    }
    async confirmMakeWriteable(resource) {
        const { confirmed } = await this.dialogService.confirm({
            ...
            detail: ( localize(12135, "Paths can be configured as read-only via settings.")),
            primaryButton: ( localize(12136, "&&Save Anyway"))
        });
        return confirmed;
    }
